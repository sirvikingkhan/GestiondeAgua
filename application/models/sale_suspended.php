...
class Sale_suspended extends CI_Model
{
	...
	function save ($items,$customer_id,$employee_id,$comment,$payments,$sale_id=false)
	{
		...
		foreach($payments as $payment_id=>$payment)
		{
			...
			(
				...
				'payment_amount'=>to_currency_no_money($payment['payment_amount'])
			);
	...
	{
		$this->db->select('sales_suspended_payments.payment_amount,sales_suspended_payments.payment_id,payments.payment_type as payment_type');
		$this->db->from('sales_suspended_payments');
		$this->db->join('payments','payments.payment_id = sales_suspended_payments.payment_id');
		$this->db->where('sale_id',$sale_id);
		return $this->db->get();
	}
